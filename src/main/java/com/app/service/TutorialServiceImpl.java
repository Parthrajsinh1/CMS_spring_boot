...
public class TutorialServiceImpl implements ITutorialService {
	...
	public List<String> getTutorialDetails(long topicId) {
		...
		return tutDao.getTutorialDetails(topicId);
	}
	...
	public Tutorial getTutorialDetails(String tutName) {
		...
		//simply update state of persistence of entity
		tutorial.setVisits(tutorial.getVisits()+1);
		return tutorial;
	}//when @Tx method rets---tx mgr chks for run time excs--no excs--tx.commit
	//-->session.flush--->hib auto dirty chking--->update query ---
	//pooled out db cn rets to the pool , session close , l1 : destroyed
